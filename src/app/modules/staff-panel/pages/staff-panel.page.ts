...
@Component({
  selector: "staff-panel-page",
  templateUrl: `./staff-panel.page.html`,
  styleUrls: ["./staff-panel.page.scss"],
})
export class StaffPanelPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.ipAddressClient.getIpDetails().subscribe(
      (_) => {
        this.registerRequest.countryPhoneCode = this.getPhoneCodeFromCountryCode(
          _.country_code
        );
      },
      (err) =>
        (this.registerRequest.countryPhoneCode = Constant.DEFAULT_PHONE_CODE)
    );

    firebase.initializeApp(environment.firebase);

    // reCaptcha
    this.phoneRecaptchaVerifier = new firebase.auth.RecaptchaVerifier(
  ...
  login() {
    ...
    this.authClient.login(this.loginRequest).subscribe(
      (res) => {
  ...
  register() {
    ...
    this.authClient.register(this.registerRequest).subscribe(
      (res) => {
        this.registerButtonText = this.LABEL_SENDING_OTP;
        this.loginResponse = res;
        this.phoneAuth(
          this.registerRequest.countryPhoneCode +
            this.registerRequest.mobile_number,
          false
        );
      },
      (err) => {
        ...
        if (err?.error?.errors?.mobile_number) {
          this.registerError.mobileNumber =
  ...
  phoneAuth(mobileNumber, isRetry) {
    ...
    this.firebaseAuth
      .signInWithPhoneNumber(mobileNumber, this.phoneRecaptchaVerifier)
      ...
      this.otpConfirmationResult
        ...
        .then((result) => {
          ...
          if (!this.showRegister) {
            ...
            this.authClient
              .verifyMobile(this.otpMobileNumber)
              .subscribe((res) => {
                this.loginRequest = new LoginRequest();
                this.authenticationService.login(res);
                this.router.navigateByUrl(this.returnUrl);
